...
class genome:
    ...
    def run_anim(self,scene,v_shift,l_shift):
        for (x,y) in zip(range(len(self.arg_seq)),self.arg_seq):
            ...
            # scene.play(self.genome.animate.shift(x*LEFT))
        ...
        scene.play(self.genome.animate.shift(2*DOWN))
        scene.wait(0.5)
...
# class MyScene(Scene):
    ...
    def make_subseq(self, scene):
            ...
            for x in range(5):
                ...
                for idx in range(6):
                    ...
                    if a == 1 and x == 2:  # Customize the condition for the gene you want to highlight
                        highlighted_gene = gene  # Set the highlighted gene to the desired gene
                    else:
        ...
        if highlighted_gene is not None:
            # scene.play(ApplyMethod(subseq.fade, 1), run_time=0.5)  # Fade out the entire subseq
            # scene.play(ApplyMethod(highlighted_gene.scale, 2), ApplyMethod(highlighted_gene.move_to, ORIGIN), run_time=0.5)
            scene.play(ApplyMethod(highlighted_gene.move_to, ORIGIN), ApplyMethod(highlighted_gene.scale, 2), run_time=0.5)  # Zoom in on the highlighted gene
            self.genome.fade(1)
