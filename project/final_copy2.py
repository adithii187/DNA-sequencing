        ...
        for a in range(3):  # make this number 5 or 7
            ...
            for x in range(5):
                ...
                for idx in range(6):
                    ...
                    self.Textdna = Tex(r"DNA sequence").shift(DOWN*2)
                    self.Textread = Tex(r"reads").shift(UR)
                    ...
                    scene.play(Create(self.Textdna))
                    scene.play(Create(self.Textread))
...
class worstcase(MovingCameraScene):
    ...
    def construct(self):
        ...
        position = genome.get_edge_center(LEFT)[0] + 0.25
        for x in range(11):
            arr = self.makearrow(position+x*0.5)
