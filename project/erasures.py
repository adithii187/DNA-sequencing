...
class erasures(Scene):
    ...
    def construct(self,scene):
        ...
        for (x,y) in zip(range(len(self.seq)),self.seq):
            ...
            # scene.play(self.obj_genome.animate.shift(x*LEFT))
            # scene.play(dna_obj.animate.shift(2*LEFT),run_time=0.5)
            scene.play(dna_obj.animate.shift((self.l_shift)*LEFT+self.v_shift*UP),run_time=0.2)
        
        

        scene.play(Transform(list_of_genes[2],list_of_erasures[2]))
        scene.play(Transform(list_of_genes[6],list_of_erasures[6]))
        scene.play(Transform(list_of_genes[7],list_of_erasures[7]))
